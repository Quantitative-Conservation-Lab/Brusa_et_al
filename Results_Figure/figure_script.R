...
ggplot() + 
geom_point(data = results, aes(x = Species, y = FF.Count, color = "Camera"), shape=16, color="red", size=5) +
...
scale_color_manual(name='',
                  ...
                  values=c('Estimate'='black', 'Camera'='red', 'Observer'='blue'))

#For averaging observers
ggplot() + 
  geom_point(data = results, aes(x = Species, y = FF.Count, color = "Camera"), shape=16, color="red", size=5) +
  geom_point(data = results, aes(x = Species, y = Obs.Ave.Count, color = "Observer"), color = "blue", size = 3) +
